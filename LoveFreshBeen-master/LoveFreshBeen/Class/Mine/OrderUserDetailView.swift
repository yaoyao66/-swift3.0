...
class OrderUserDetailView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        collectionButton.setTitle("+ 收藏", for: UIControlState())
        collectionButton.setTitleColor(LFBTextBlackColor, for: UIControlState())
        collectionButton.setTitle("取消收藏", for: .selected)
        collectionButton.setTitleColor(UIColor.white, for: .selected)
        collectionButton.setBackgroundImage(UIImage.imageWithColor(LFBNavigationYellowColor, size: CGSize(width: 60, height: 25), alpha: 1), for: UIControlState())
